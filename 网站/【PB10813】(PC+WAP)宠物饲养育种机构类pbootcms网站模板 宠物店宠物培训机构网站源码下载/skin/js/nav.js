$(document).ready(function(){
    ...
    var con2_bigbox = $(".con2_bigbox");
    con2_bigbox.find("div").eq(0).hide();
    con2_bigbox.find("p").click(function(){
        var ind = $(this).index();
        console.log(ind);
        if(!con2_bigbox.find("div").is(":animated")){
            con2_bigbox.find("div").stop(false,false).animate({"width":"0"},500).css("display","none");
            con2_bigbox.find("div").eq(ind/2).stop(false,false).css("display","block").animate({"width":"480px"},500);
        }
    });
